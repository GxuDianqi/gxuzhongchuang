...
async function afterLoginSuccess(email, sourceHint) {
  ...
  //   · OTP 登录成功 → 默认跳 register.html（报名页顶部第 ① 组就是「设置登录密码」，确保用户首次能看到设置入口）
  //   · 密码登录成功 → 默认跳 index.html（首页）
  let target;
  if (admin) target = 'admin.html';
  else if (redirect) target = redirect;
  else if (sourceHint.includes('OTP')) target = 'register.html';
  else target = 'index.html';

  showAlert(alertEl, 'success', admin
    ? `✅ ${sourceHint}：管理员登录成功，正在进入审批后台...`
    : `✅ ${sourceHint}：登录成功${sourceHint.includes('OTP') ? '，正在前往报名表（可立即设置登录密码）' : '，正在跳转首页'}...`);
