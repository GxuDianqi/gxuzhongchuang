 ...
 *                  → 忘记密码点底部链接，邮箱发重置邮件
 * 【Tab 2 · 备选】邮箱 + 6 位验证码 OTP：新生首次登录 = 自动注册 + 验证邮箱
 *                  → 登录成功后可在报名页设置密码
 *
 * 附加：
 *   · 任意登录方式成功后，邮箱自动保存到 localStorage.last_login_email
 *   · 下次打开登录页自动预填邮箱框（用户可修改）
 *   · 不自动跳走（用户要求"第二次用账号密码登录，而不是保存上次的记录直接进入"）
 * ======================================================== */
...
function friendlyErrMsg(err) {
  ...
  if (/otp.*(expired|invalid|mismatch)|验证码.*(过期|错误|不匹配)/i.test(raw)) {
    return '❌ 验证码已过期或错误：请重新点"发送验证码"获取新的 6 位码。';
  }
  return '❌ ' + raw;
}
...
function saveLastEmail(email) {
  try { if (email) localStorage.setItem(LAST_EMAIL_KEY, String(email).trim()); }
  catch (_) { /* localStorage 不可用忽略 */ }
}

// ---------- 登录成功后统一处理 ----------
async function afterLoginSuccess(email, sourceHint) {
  saveLastEmail(email);  // ✅ 保存下次自动预填
  ...
  if (otpPhase === 'send') {
    ...
    try {
      ...
      showAlert(alertEl, 'success',
        `✅ 验证码已发送至 ${email}，请到邮箱（含垃圾箱）中找到我们的邮件，复制其中的 6 位验证码粘贴到上方。`);
    } catch (err) {
...
forgotLink?.addEventListener('click', async (e) => {
  e.preventDefault();
  const email = (pwdEmailInput.value || otpEmailInput.value || loadLastEmail() || '').trim();
  if (!email) {
...
// ========================================================
// 页面启动：预填上次登录的邮箱 → 按 URL 参数决定初始 Tab → 不自动跳走
// ========================================================
(async () => {
  // ① 预填邮箱（两个 Tab 的邮箱输入框同步）
  const lastEmail = loadLastEmail();
  if (lastEmail) {
    pwdEmailInput.value = lastEmail;
    otpEmailInput.value = lastEmail;
  }

  // ② 初始 Tab 判断：
  //   · URL 带 ?mode=otp  → 从报名页"切换邮箱"过来，需要 OTP
  //   · 有 lastEmail      → 默认密码登录 Tab（用户已经设置过密码的概率高）
  ...
  try {
    ...
    if (user) {
      showAlert(alertEl, 'info',
        ...
        '如需切换账号请先「退出登录」，或直接点击下方按钮继续前往：<br/>' +
        `<a href="index.html" style="color:var(--accent-cyan);margin-right:14px;">🏠 前往首页</a>` +
        `<a href="register.html" style="color:var(--accent-cyan);">📝 填写报名表</a>`, 9999999);
    }
  } catch (err) {
